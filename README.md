...
- **两个前端**：验证台（`index.html`，由后端直接服务）与 2048 自动对局页（`2048_laya.html`，含抉择历史留痕与 JSONL 导出）。
...
flowchart LR
    ...
    API --> U2["2048_laya.html<br/>自动对局 + 留痕"]
```
...
直接双击打开（`file://` 直连 `127.0.0.1:8771`；后端已开 CORS）。每一步发一次
`POST /api/predict`，**一次前向同时回答 3 个 typed question**（不加钱）：

| question | 类型 | 作用 |
|---|---|---|
| `move` | `choice` | 走哪一步 —— 决策输出 |
| `crowding` | `score` | 拥挤度探针（与真值比对） |
| `merge_available` | `noul` | 能否合并探针（与真值比对） |

前端打印的**抉择历史**每条包含：步号、应用方向、模型的原始选择与概率、合法性/回退徽章、
得分增量、新方块、移动前后两张 4×4 缩略棋盘、四方向概率条、真值对比、tokens 与延迟；
支持「仅看异常/回退」筛选、`导出 JSONL`（21 个顶层字段，含 `board_before/after`）与一键复制。

模型给出**非法方向**时取 `ranking` 中置信度最高的合法方向，并在留痕上标 `fallback: true`
（不做这层回退会直接卡死）。
...
├── 2048_laya.html          # 2048 自动对局 + 抉择留痕（~840 行，file:// 直连 8771）
...
A：不是。决策通道未对齐（见[已知限制](#已知限制)），模型会随机游走并很快堵死，这是**预期结果**。
